          ...
          {documents.map((doc) => (
            <li key={doc.id} className="flex items-center justify-between rounded border border-slate-200 p-3">
              <div>
                ...
                <p className="text-xs text-slate-500">{doc.mime} · {(doc.size / 1024 / 1024).toFixed(2)} MB</p>
              </div>
      ...
      <div className="flex items-center justify-between text-xs text-slate-500">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`rounded border px-2 py-1 ${hasPrevDocs ? "border-slate-300" : "border-slate-200 text-slate-400"}`}
          >
            Previous
          </button>
          <button
            ...
            className={`rounded border px-2 py-1 ${hasNextDocs ? "border-slate-300" : "border-slate-200 text-slate-400"}`}
          >
            Next
          </button>
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {invoices.map((invoice) => (
                <tr key={invoice.id}>
                  ...
                  <td className={`px-3 py-2 text-right capitalize ${invoice.status === "paid" ? "text-emerald-600" : invoice.status === "overdue" ? "text-red-600" : "text-slate-600"}`}>
                    {invoice.status}
                  </td>
      ...
      <div className="flex items-center justify-between text-xs text-slate-500">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`rounded border px-2 py-1 ${hasPrevInvoices ? "border-slate-300" : "border-slate-200 text-slate-400"}`}
          >
            Previous
          </button>
          <button
            ...
            className={`rounded border px-2 py-1 ${hasNextInvoices ? "border-slate-300" : "border-slate-200 text-slate-400"}`}
          >
            Next
          </button>
        </div>
      ...
      <div className="space-y-4">
        {threads.length > 1 ? (
          <div className="flex flex-wrap items-center gap-3">
            <label className="text-xs font-medium text-slate-500">Conversation</label>
            <select
              ...
              {messages.map((message) => {
                ...
                return (
                  <li key={message.id} className={`flex ${isClient ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[80%] rounded-lg px-3 py-2 ${isClient ? "bg-primary-600 text-white" : "bg-white text-slate-700 shadow"}`}>
                      <p>{message.body}</p>
